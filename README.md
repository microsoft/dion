...
* `rmnp.py`: RMNP (Row-Momentum Normalized Preconditioning) replaces Muon's Newton-Schulz iteration with a single row-wise (input-dimension) ℓ₂ normalization of the momentum update, cutting the per-iteration cost from `O(mn·min(m,n))` to `O(mn)` for an `m×n` weight. It reuses Muon's momentum, distributed assembly, and weight update, so it takes the same arguments as `muon.py` (minus the Newton-Schulz-specific options). Following the paper, it scales the learning rate by the original Muon adjustment `max(1, sqrt(d_out/d_in))` by default (`adjust_lr="spectral_norm_clip"`). Because row normalization is cheap and well-defined on any 2D weight, RMNP can additionally optimize the embedding and LM-head matrices — not only the hidden weights. See [the paper](https://arxiv.org/abs/2603.20527) for more details.
