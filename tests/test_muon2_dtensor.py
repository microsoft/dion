...
def _row_sharded_worker(rank: int, world_size: int, port: int) -> None:
    ...
    try:
        ...
        local_variance = (
            variance.to_local() if isinstance(variance, DTensor) else variance
        )
        expected = (1 - beta2) * (local_grad * local_grad)
        assert local_variance.shape == local_param.shape
        torch.testing.assert_close(local_variance, expected)
    finally:
        dist.destroy_process_group()


@pytest.mark.skipif(CUDA_DEVICE_COUNT < 2, reason="needs >= 2 CUDA devices")
def test_muon2_row_sharded_dtensor_variance_is_local():
    mp.spawn(_row_sharded_worker, args=(2, 29624), nprocs=2, join=True)
